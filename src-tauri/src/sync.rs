...
pub struct SyncTask {
    ...
    /// Defaulted for the same reason as the estimate; a row from a pre-board
    /// client lands in the first column, where an unplaced task belongs anyway.
    #[serde(default)]
...
fn collect_changes(conn: &Connection, since: &str) -> rusqlite::Result<SyncBundle> {
    ...
    let mut stmt = conn.prepare(
        "SELECT id, title, notes, due_date, remind_at, status, priority, created_at,
                ...
                stage, board_index, updated_at, deleted_at
         FROM tasks",
    )?;
            ...
            conn.execute(
                "INSERT INTO tasks (id, title, notes, due_date, remind_at, status, priority,
                 ...
                 ON CONFLICT(id) DO UPDATE SET
                    ...
                    stage = excluded.stage, board_index = excluded.board_index,
                    updated_at = excluded.updated_at, deleted_at = excluded.deleted_at",
                params![
                    t.id,
                    t.title,
...
mod tests {
    ...
    #[test]
    fn board_placement_survives_a_full_sync_round_trip() {
        let conn = setup();
        let pulled = SyncBundle {
            tasks: vec![SyncTask {
                id: "t1".into(),
        ...
        let bundle = collect_changes(&conn, EPOCH).unwrap();
        let task = bundle.tasks.iter().find(|t| t.id == "t1").unwrap();
        assert_eq!(task.stage.as_deref(), Some("doing"));
        assert_eq!(task.board_index, 42.5);
    }

    /// A cloud row written before these features has no `estimate_minutes`,
    /// `stage` or `board_index` key at all; the whole pull must not fail on it.
    #[test]
    fn task_json_without_an_estimate_still_deserializes() {
        ...
        assert_eq!(task.stage, None, "an unplaced task belongs in column one");
        assert_eq!(task.board_index, 0.0);
    }
